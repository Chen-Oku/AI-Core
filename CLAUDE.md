...
alembic upgrade head      # apply pending schema migrations -- required before first run
...
- Installed today (from `.venv`): `fastapi`, `uvicorn`, `pydantic`, `ollama`, `httpx`, `pytest`, `sqlalchemy`, `psycopg[binary]`, `pydantic-settings`, `chromadb-client`, `alembic`.
- Tests live in `backend/tests/`, mirroring the `app/` package layout (e.g. `tests/memory/` covers `app/memory/`, `tests/auth/` covers `app/auth/`). Memory/auth tests point SQLAlchemy at a temporary SQLite file and call `app/memory/db.py:init_db()` (`create_all()`) directly, bypassing Alembic entirely — they don't need Postgres running or migrations applied. `tests/rag/test_chroma_vector_store.py` needs a real Chroma server reachable (see below) — `chromadb-client` has no embedded/local mode, unlike SQLAlchemy. `tests/rag/test_rag_service.py`, `tests/rag/test_chunker.py`, `tests/services/test_chat_service.py`, `tests/services/test_agent_service.py`, `tests/services/test_auth_service.py`, and `tests/services/test_image_service.py` use fakes and need no infra. There is no lint/format tooling configured yet.
- Running the app for real needs a reachable Postgres at `DATABASE_URL` and a reachable Chroma server at `CHROMA_HOST`/`CHROMA_PORT` (`backend/.env`, see `backend/.env.example`): `docker compose -f docker/docker-compose.yml up -d`. **The app no longer creates its own schema** (ADR-010) — run `alembic upgrade head` against that Postgres first, or `/chat`/`/agent`/key creation will fail with "relation does not exist". Schema changes go through `alembic revision --autogenerate -m "..."` (review the generated script) + `alembic upgrade head`, not by editing a model and restarting.
- Every endpoint requires an `X-API-Key` header (`401` otherwise, ADR-009). Provision one with `python backend/scripts/create_api_key.py <tenant-name>` (after `alembic upgrade head`) — prints the raw key once, only its hash is stored. No admin endpoint creates keys (chicken-and-egg: nothing is authenticated yet to call one).
...
- `app/core/` — cross-cutting config, e.g. [config.py](backend/app/core/config.py) (`pydantic-settings`) reads `DATABASE_URL`, `CHROMA_HOST`, `CHROMA_PORT`, `EMBEDDING_MODEL`, `AUTOMATIC1111_BASE_URL`, `RAG_CHUNK_SIZE`, `RAG_CHUNK_OVERLAP` from the environment/`.env`; [db_base.py](backend/app/core/db_base.py) holds the single declarative `Base` shared by `app/memory/models.py` and `app/auth/models.py`; [db_models.py](backend/app/core/db_models.py) imports both ORM models purely to register them with `Base.metadata` — `alembic/env.py` imports it for `--autogenerate` to see the full schema.
- `alembic/` — schema migrations (ADR-010, `docs/modules/Memory.md`): `env.py` points `target_metadata` at the shared `Base.metadata` and the connection URL at `settings.database_url`; `versions/` holds one migration today (`1af1ac37aa0c_initial_schema.py`, creating `messages`/`api_keys` from nothing). The app itself no longer creates its schema — run `alembic upgrade head` before starting it.
